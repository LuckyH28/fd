...
import com.aisino.domain.common.enums.EShopCertificateBytesInfo;
//import com.aisino.domain.common.util.ProtocolFactory;
...
public final class PKCS7 {
	private final static Logger LOGGER = LogManager.getLogger(PKCS7.class);
...
//		System.load(CaConstant.getProperty("DLLADDRESS1"));
	...
	public static synchronized PKCS7 getPkcs(){
		PKCS7 pkcs = new PKCS7();
		EShopCertificateBytesInfo certificate = null;
		try {
			certificate = getPlatformCertificateEntity();
		} catch (IOException e) {
		...
		pkcs.setTrusts(new String(certificate.getTrustsBytes()));
		pkcs.setDecryptPfx(certificate.getPrivatePFXBytes(), certificate.getPrivatePFXKey());
		pkcs.setSignedPfx(certificate.getPrivatePFXBytes(), certificate.getPrivatePFXKey());
		return pkcs;
	}
	
	
//	public static void setpfx(String nsrsbh){
//		EShopCertificateBytesInfo certificate = null;
//		try {
//			certificate = getPlatformCertificateEntity(nsrsbh);
//		} catch (IOException e) {
//			LOGGER.error("获取平台证书实体对象时出现异常，详情为：" + e);
//			e.printStackTrace();
//		}
...
//	}
	...
	private static EShopCertificateBytesInfo getPlatformCertificateEntity()
		...
		// 读取企业平台私钥证书，并注入到EShopCertificateBytesInfo对象中
		String pfxurl = CaConstant.SL_CA_PRIVATE_KEY;
		certificateBytesInfo.setPrivatePFXBytes(readFileToByteArray(
				new File(pfxurl)));
		// 读取企业平台公钥证书，并注入到EShopCertificateBytesInfo对象中
		String pfxurl2 = CaConstant.SL_CA_PUBLIC_KEY;
		certificateBytesInfo.setPublicPFXBytes(readFileToByteArray(
				new File(pfxurl2)));
		// 读取企业平台证书信任链，并注入到EShopCertificateBytesInfo对象中
		String pfxurl3 = CaConstant.SL_CA_TRUST;
		certificateBytesInfo.setTrustsBytes(readFileToByteArray(
				new File(pfxurl3)));
		// 读取企业平台私钥证书密码，并注入到EShopCertificateBytesInfo对象中
		String pfxurl4 = CaConstant.getCAPassword();
		certificateBytesInfo.setPrivatePFXKey(pfxurl4);
		//certificateBytesInfo.setPrivatePFXKey("96766465475");
